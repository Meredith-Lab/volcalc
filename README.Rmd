...
```
It is still in a stage of development likely to introduce many breaking changes.
...
Volatility can be estimated for most chemical compounds using a .mol file.
The [KEGG](https://www.genome.jp/kegg/) database provides .mol files for most compounds and they can be downloaded using just the [KEGG unique identifier](https://www.genome.jp/kegg/compound/) for the compound of interest.
...
Volatility estimation is done using a modified version of the SIMPOL.1 method (Pankow & Asher 2008).
...
Installation of `volcalc` requires the system libraries [OpenBabel](https://openbabel.org/) and Eigen3 (requirements of the `ChemmineOB` package, which `volcalc` depends on).
`pak` will take care of the installation of these libraries for you on some systems, but you may need to install them manually.
...
SIMPOL.1: a simple group contribution method for predicting vapor pressures and enthalpies of vaporization of multifunctional organic compounds. Atmos. Chem. Phys.
