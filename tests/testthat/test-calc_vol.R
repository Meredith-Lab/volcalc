test_that("volatility estimate is correct for example compound for entire workflow", {
  ...
  expect_equal(round(ex_vol_df$volatility, 6), 6.963971)
})
...
test_that("returns correct number of columns depending on return arguments", {
  expect_equal(ncol(calc_vol("map00361", "data", compound_id = "C16181")), 6)
  expect_equal(ncol(calc_vol("map00361", "data", compound_id = "C16181", return_fx_groups = TRUE)), 44)
  expect_equal(ncol(calc_vol("map00361", "data", compound_id = "C16181", return_calc_steps = TRUE)), 9)
  expect_equal(ncol(calc_vol("map00361", "data", compound_id = "C16181", return_fx_groups = TRUE, return_calc_steps = TRUE)), 47)
})
